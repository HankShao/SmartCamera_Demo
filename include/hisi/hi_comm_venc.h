 ...
 * Description: chip specific configure data structure of venc
...
#define HI_VENC_SCALING_DIMENSION_NUM  2
...
#define HI_VENC_PRORES_QT_COEF_NUM     64
#define HI_VENC_PRORES_MAX_ID_CHAR_NUM 4
#define HI_VENC_PRORES_MAX_VENDOR_NUM  4
...
typedef struct hiVENC_PACK_S {
    ...
    VENC_PACK_INFO_S     stPackInfo[HI_VENC_MAX_PACK_INFO_NUM];              /* R; the stream packet Information */
} VENC_PACK_S;
...
typedef struct hiVENC_STREAM_INFO_H264_S {
    ...
    HI_U32                 u32UpdateAttrCnt;       /* R; Number of times that channel attributes or parameters
...
typedef struct hiVENC_STREAM_INFO_H265_S {
    ...
    HI_U32                 u32UpdateAttrCnt;     /* R; Number of times that channel attributes or
...
typedef struct hiVENC_STREAM_ADVANCE_INFO_H264_S {
    ...
    VENC_SSE_INFO_S    stSSEInfo[HI_VENC_MAX_SSE_NUM];                         /* R; the information of the sse */
...
typedef struct hiVENC_MPF_CFG_S {
    ...
    SIZE_S  astLargeThumbNailSize[HI_VENC_MAX_MPF_NUM]; /* RW; The resolution of large ThumbNail */
} VENC_MPF_CFG_S;
...
typedef struct hiVENC_ATTR_PRORES_S {
    HI_CHAR             cIdentifier[HI_VENC_PRORES_MAX_ID_CHAR_NUM];
...
typedef struct hiVENC_GOP_DUALP_S {
    HI_U32 u32SPInterval;            /* RW; Range:[0, 65536]; Interval of the special P frames,
...
typedef struct hiVENC_RECV_PIC_PARAM_S {
    HI_S32 s32RecvPicNum;                         /* RW; Range:[-1, 2147483647]; Number of frames received and
...
typedef struct hiVENC_CHN_STATUS_S {
    ...
    HI_U32 u32LeftRecvPics;                       /* R; Number of frames to be received. This member is valid
                                                    after HI_MPI_VENC_StartRecvPicEx is called. */
    HI_U32 u32LeftEncPics;                        /* R; Number of frames to be encoded. This member is valid
                                                    after HI_MPI_VENC_StartRecvPicEx is called. */
...
typedef struct hiVENC_H264_SLICE_SPLIT_S {
    HI_BOOL bSplitEnable;                         /* RW; Range:[0, 1]; slice split enable, HI_TRUE:enable,
                                                    HI_FALSE:diable, default value:HI_FALSE */
    HI_U32  u32MbLineNum;                         /* RW; the max number is (Picture height + 15)/16;
...
typedef struct hiVENC_H264_INTRA_PRED_S {
    HI_U32     constrained_intra_pred_flag;       /* RW; Range:[0, 1];default: HI_FALSE,
...
typedef struct hiVENC_H264_TRANS_S {
    HI_U32     u32IntraTransMode;               /* RW; Range:[0, 2]; Conversion mode for intra-prediction,
                                                    0: trans4x4, trans8x8; 1: trans4x4, 2: trans8x8 */
    HI_U32     u32InterTransMode;               /* RW; Range:[0, 2]; Conversion mode for inter-prediction,
                                                    0: trans4x4, trans8x8; 1: trans4x4, 2: trans8x8 */

    HI_BOOL    bScalingListValid;               /* RW; Range:[0, 1]; enable Scaling, default: HI_FALSE  */
    /* RW; Range:[1, 255]; A quantization table for 8x8 inter-prediction */
    HI_U8      InterScalingList8X8[HI_VENC_SCALING_LIST_SIZE_64];
    /* RW; Range:[1, 255]; A quantization table for 8x8 intra-prediction */
    HI_U8      IntraScalingList8X8[HI_VENC_SCALING_LIST_SIZE_64];

    HI_S32     chroma_qp_index_offset;          /* RW; Range:[-12, 12];default value: 0,
                                                    see the H.264 protocol for the meaning */
} VENC_H264_TRANS_S;

/* the param of the h264e entropy */
typedef struct hiVENC_H264_ENTROPY_S {
    HI_U32 u32EntropyEncModeI;                  /* RW; Range:[0, 1]; Entropy encoding mode for the I frame,
                                                    0:cavlc, 1:cabac */
    HI_U32 u32EntropyEncModeP;                  /* RW; Range:[0, 1]; Entropy encoding mode for the P frame,
                                                    0:cavlc, 1:cabac */
    HI_U32 u32EntropyEncModeB;                  /* RW; Range:[0, 1]; Entropy encoding mode for the B frame,
                                                    0:cavlc, 1:cabac */
...
typedef struct hiVENC_H264_VUI_TIME_INFO_S {
    HI_U8  timing_info_present_flag;          /* RW; Range:[0, 1];
                                                If 1, timing info belows will be encoded into vui. */
    HI_U8  fixed_frame_rate_flag;             /* RW; Range:[0, 1];
                                                see the H.264 protocol for the meaning. */
    HI_U32 num_units_in_tick;                 /* RW; Range:(0, 4294967295];
                                                see the H.264 protocol for the meaning */
    HI_U32 time_scale;                        /* RW; Range:(0, 4294967295];
                                                see the H.264 protocol for the meaning */
} VENC_VUI_H264_TIME_INFO_S;

/* the param of the vui aspct ratio */
typedef struct hiVENC_VUI_ASPECT_RATIO_S {
    HI_U8  aspect_ratio_info_present_flag;    /* RW; Range:[0, 1]; If 1, aspectratio info belows
                                                will be encoded into vui */
    HI_U8  aspect_ratio_idc;                  /* RW; Range:[0, 255]; 17~254 is reserved,
                                                see the protocol for the meaning. */
    HI_U8  overscan_info_present_flag;        /* RW; Range:[0, 1]; If 1,
...
typedef struct hiVENC_JPEG_PARAM_S {
    ...
    HI_U8  u8YQt[HI_VENC_JPEG_QT_COEF_NUM];                 /* RW; Range:[1, 255]; Y quantization table */
    HI_U8  u8CbQt[HI_VENC_JPEG_QT_COEF_NUM];                /* RW; Range:[1, 255]; Cb quantization table */
    HI_U8  u8CrQt[HI_VENC_JPEG_QT_COEF_NUM];                /* RW; Range:[1, 255]; Cr quantization table */
    HI_U32 u32MCUPerECS;              /* RW; the max MCU number is (picwidth + 15) >> 4 x (picheight + 15) >> 4 x 2];
                                        MCU number of one ECS */
} VENC_JPEG_PARAM_S;

/* the param of the mjpege */
typedef struct hiVENC_MJPEG_PARAM_S {
    HI_U8 u8YQt[HI_VENC_MJPEG_QT_COEF_NUM];                  /* RW; Range:[1, 255]; Y quantization table */
    HI_U8 u8CbQt[HI_VENC_MJPEG_QT_COEF_NUM];                 /* RW; Range:[1, 255]; Cb quantization table */
    HI_U8 u8CrQt[HI_VENC_MJPEG_QT_COEF_NUM];                 /* RW; Range:[1, 255]; Cr quantization table */
    HI_U32 u32MCUPerECS;              /* RW; the max MCU number is (picwidth + 15) >> 4 x (picheight + 15) >> 4 x 2];
                                        MCU number of one ECS */
} VENC_MJPEG_PARAM_S;
...
typedef struct hiVENC_PRORES_PARAM_S {
    HI_U8 u8LumaQt[HI_VENC_PRORES_QT_COEF_NUM];               /* RW; Range:[1, 255]; Luma quantization table */
    HI_U8 u8ChromaQt[HI_VENC_PRORES_QT_COEF_NUM];             /* RW; Range:[1, 255]; Chroma quantization table */
    HI_CHAR encoder_identifier[HI_VENC_PRORES_MAX_VENDOR_NUM];    /* RW: identifies the encoder vendor or product that
                                        generated the compressed frame */
} VENC_PRORES_PARAM_S;

/* the attribute of the roi */
typedef struct hiVENC_ROI_ATTR_S {
    HI_U32  u32Index;                     /* RW; Range:[0, 7]; Index of an ROI.
                                            The system supports indexes ranging from 0 to 7 */
    HI_BOOL bEnable;                      /* RW; Range:[0, 1]; Whether to enable this ROI */
    HI_BOOL bAbsQp;                       /* RW; Range:[0, 1]; QP mode of an ROI.
                                            HI_FALSE: relative QP.HI_TURE: absolute QP */
    HI_S32  s32Qp;                        /* RW; Range:[-51, 51];
...
typedef struct hiVENC_ROI_ATTR_EX_S {
    HI_U32  u32Index;                     /* RW; Range:[0, 7]; Index of an ROI.
                                            The system supports indexes ranging from 0 to 7 */
    HI_BOOL bEnable[HI_VENC_ROI_FOR_BUTT];                   /* RW; Range:[0, 1]; Subscript of array
                                            0: I Frame; 1: P/B Frame; 2: VI Frame; other params are the same */
    HI_BOOL bAbsQp[HI_VENC_ROI_FOR_BUTT];                    /* RW; Range:[0, 1]; QP mode of an ROI.
                                            HI_FALSE: relative QP.HI_TURE: absolute QP */
...
typedef struct hiVENC_ROIBG_FRAME_RATE_S {
    HI_S32 s32SrcFrmRate;                 /* RW; Range:[-1, 2147483647];Source frame rate of a non-ROI,
                                            can not be configured 0 */
    HI_S32 s32DstFrmRate;                 /* RW; Range:[-1, 2147483647];Target frame rate of a non-ROI,
...
typedef struct hiVENC_REF_PARAM_S {
    ...
    HI_BOOL      bEnablePred;           /* RW; Range:[0, 1]; Whether some frames at the base layer are referenced
                                            by other frames at the base layer. When bEnablePred is HI_FALSE,
...
typedef struct hiVENC_H265_SLICE_SPLIT_S {
    HI_BOOL bSplitEnable;                          /* RW; Range:[0, 1]; slice split enable,
                                                    HI_TRUE:enable, HI_FALSE:diable, default value:HI_FALSE */
    HI_U32  u32LcuLineNum;                         /* RW; Range:(Picture height + lcu size minus one)/lcu size;
...
typedef struct hiVENC_H265_TRANS_S {
    ...
    /* RW; Range:[1, 255]; Scaling List for inter 4X4 block. */
    HI_U8   InterScalingList4X4[HI_VENC_SCALING_DIMENSION_NUM][HI_VENC_SCALING_LIST_SIZE_16];
    /* RW; Range:[1, 255]; Scaling List for intra 4X4 block. */
    HI_U8   IntraScalingList4X4[HI_VENC_SCALING_DIMENSION_NUM][HI_VENC_SCALING_LIST_SIZE_16];

    HI_BOOL bScalingListTu8Valid;             /* RW; Range:[0, 1]; If 1, ScalingList8X8 belows will be encoded. */
    /* RW; Range:[1, 255]; Scaling List for inter 8X8 block. */
    HI_U8   InterScalingList8X8[HI_VENC_SCALING_DIMENSION_NUM][HI_VENC_SCALING_LIST_SIZE_64];
    /* RW; Range:[1, 255]; Scaling List for intra 8X8 block. */
    HI_U8   IntraScalingList8X8[HI_VENC_SCALING_DIMENSION_NUM][HI_VENC_SCALING_LIST_SIZE_64];

    HI_BOOL bScalingListTu16Valid;            /* RW; Range:[0, 1]; If 1, ScalingList16X16 belows will be encoded. */
    /* RW; Range:[1, 255]; Scaling List for inter 16X16 block. */
    HI_U8   InterScalingList16X16[HI_VENC_SCALING_DIMENSION_NUM][HI_VENC_SCALING_LIST_SIZE_64];
    /* RW; Range:[1, 255]; Scaling List for inter 16X16 block. */
    HI_U8   IntraScalingList16X16[HI_VENC_SCALING_DIMENSION_NUM][HI_VENC_SCALING_LIST_SIZE_64];

    HI_BOOL bScalingListTu32Valid;            /* RW; Range:[0, 1]; If 1, ScalingList32X32 belows will be encoded. */
    /* RW; Range:[1, 255]; Scaling List for inter 32X32 block. */
    HI_U8   InterScalingList32X32[HI_VENC_SCALING_LIST_SIZE_64];
    /* RW; Range:[1, 255]; Scaling List for inter 32X32 block. */
    HI_U8   IntraScalingList32X32[HI_VENC_SCALING_LIST_SIZE_64];
} VENC_H265_TRANS_S;
...
typedef struct hiVENC_H265_DBLK_S {
    ...
    HI_U32 loop_filter_across_tiles_enabled_flag;  /* RW; Range:[0, 1]; see the H.265 protocol for the meaning. */
    HI_U32 loop_filter_across_slices_enabled_flag; /* RW; Range:[0, 1]; see the H.265 protocol for the meaning. */
} VENC_H265_DBLK_S;

/* the param of the h265e sao */
typedef struct hiVENC_H265_SAO_S {
    HI_U32  slice_sao_luma_flag;      /* RW; Range:[0, 1]; Indicates whether SAO filtering is performed on the
                                        luminance component of the current slice. */
    HI_U32  slice_sao_chroma_flag;    /* RW; Range:[0, 1]; Indicates whether SAO filtering is performed on
...
typedef struct hiVENC_INTRA_REFRESH_S {
    HI_BOOL                     bRefreshEnable;     /* RW; Range:[0, 1]; intra refresh enable,
                                                        HI_TRUE:enable, HI_FALSE:diable, default value:HI_FALSE */
    VENC_INTRA_REFRESH_MODE_E   enIntraRefreshMode; /* RW; The mode of intra refresh */
    HI_U32                      u32RefreshNum;      /* RW; Number of rows/column to be refreshed
...
typedef struct hiVENC_MOD_H264E_S {
    ...
    HI_U32          u32UserDataMaxLen;      /* RW; Range:[0, 65536]; one user data buffer len */
} VENC_MOD_H264E_S;
...
typedef struct hiUSER_RC_INFO_S {
    HI_BOOL bQpMapValid;           /* RW; Range:[0, 1];
                                    Indicates whether the QpMap mode is valid for the current frame */
    HI_BOOL bSkipWeightValid;      /* RW; Range:[0, 1];
...
typedef enum {
    HI_VENC_MOSAIC_MODE_0 = 0, /* user input rect */
    HI_VENC_MOSAIC_MODE_1,     /* user input squares mosaic map */
    HI_VENC_MOSAIC_MODE_2,     /* user input user-defined shape map */
    HI_VENC_MOSAIC_MODE_BUTT
} HI_VENC_MOSAIC_MODE_E;

typedef enum {
    HI_VENC_MOSAIC_SIZE_4x4 = 0,
    HI_VENC_MOSAIC_SIZE_8x8,
    HI_VENC_MOSAIC_SIZE_16x16,
    HI_VENC_MOSAIC_SIZE_32x32,
    HI_VENC_MOSAIC_SIZE_64x64,
    HI_VENC_MOSAIC_SIZE_128x128,
    HI_VENC_MOSAIC_SIZE_BUTT
} HI_VENC_MOSAIC_BLOCK_SIZE_E;

typedef struct {
    HI_U8 u8DataY;
    HI_U8 u8DataU;
    HI_U8 u8DataV;
} VENC_MOSAIC_PIXEL_YUV;

typedef struct {
    HI_U32 u32RectNum;
    RECT_S stRect[HI_VENC_MAX_MOSAIC_RECT_NUM];
} VENC_MOSAIC_MODE0_PARAM_S;

typedef struct {
    HI_BOOL bMosaicMapValid;
    HI_PHYS_ADDR_T MosaicMapPhysAddr;
} VENC_MOSAIC_MODE1_PARAM_S;

typedef struct {
    HI_BOOL bMosaicMapValid;
    HI_PHYS_ADDR_T MosaicMapPhysAddr;
    VENC_MOSAIC_PIXEL_YUV stPixelYuv;
} VENC_MOSAIC_MODE2_PARAM_S;

/* the information of the mosaic area */
typedef struct {
    HI_VENC_MOSAIC_MODE_E enMosaicMode;
    HI_VENC_MOSAIC_BLOCK_SIZE_E enSizeMode;

    union {
        VENC_MOSAIC_MODE0_PARAM_S stMode0Param; /* params for HI_VENC_MOSAIC_MODE_0 */
        VENC_MOSAIC_MODE1_PARAM_S stMode1Param; /* params for HI_VENC_MOSAIC_MODE_1 */
        VENC_MOSAIC_MODE2_PARAM_S stMode2Param; /* params for HI_VENC_MOSAIC_MODE_2 */
    };
} MOSAIC_INFO_S;

/* the information of the user frame */
typedef struct hiMULTI_FRAME_INFO_S {
    HI_U32 u32FrameNum;
    VIDEO_FRAME_INFO_S stFrame[HI_VENC_MAX_FRAME_NUM];
    MOSAIC_INFO_S stMosaicInfo;
} MULTI_FRAME_INFO_S;

typedef struct hiVENC_CHN_CONFIG_S {
    HI_BOOL bMosaicEnable;
    HI_BOOL bCompositeEncEnable;
    HI_U32 u32QualityLevel; /* 0: lower quality; 1: higher quality. */
} VENC_CHN_CONFIG_S;
...
typedef struct hiVENC_FOREGROUND_PROTECT_S {
    ...
    HI_U32  u32ForegroundDirectionThresh;              /* RW; Range:[0, 16];
    ...
    HI_U32  u32ForegroundThreshP[RC_TEXTURE_THR_SIZE]; /* RW; Range:[0, 255]; Mad threshold for controlling
                                                        the foreground macroblock-level bit rate of P frames */
    HI_U32  u32ForegroundThreshB[RC_TEXTURE_THR_SIZE]; /* RW; Range:[0, 255]; Mad threshold for controlling
...
typedef struct hiVENC_SKIP_BIAS_S {
    HI_BOOL bSkipBiasEn;             /* RW; Range:[0, 1];
                                        Flag indicating whether the skip tendency function is enabled */
    HI_U32  u32SkipThreshGain;       /* RW; Range:[0, 15];
                                        used to calculate the SAD threshold for foreground detection */
    HI_U32  u32SkipThreshOffset;     /* RW; Range:[0, 255];
                                        used to calculate the SAD threshold for foreground detection */
    HI_U32  u32SkipBackgroundCost;   /* RW; Range:[0, 15];
                                        Skip tendency adjustment in the background */
    HI_U32  u32SkipForegroundCost;   /* RW; Range:[0, 15];
...
typedef struct hiVENC_HIERARCHICAL_QP_S {
    ...
    HI_S32      s32HierarchicalQpDelta[HI_VENC_MAX_HIERARCHY_NUM];
    /* RW; Range:[0, 5];    Number of frames at each layer */
    HI_S32      s32HierarchicalFrameNum[HI_VENC_MAX_HIERARCHY_NUM];
...
typedef struct {
    VENC_SVC_MAP_PARAM_S  stFgRegion[SVC_RECT_TYPE_BUTT];
...
typedef struct {
    ...
    RECT_S stRectAttr[HI_VENC_MAX_SVC_RECT_NUM];
    VENC_SVC_RECT_TYPE_E enDetectType[HI_VENC_MAX_SVC_RECT_NUM];
    HI_U64 u64Pts;
} VENC_SVC_RECT_INFO_S;

typedef struct {
    HI_U32 u32ParamSetId; /* Range: for H264: [0, 30]; for H265: [0, 15]; */
} VENC_PARAM_SET_ID_S;
