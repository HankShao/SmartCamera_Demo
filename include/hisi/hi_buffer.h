    ...
    COMPRESS_MODE_E enCmpMode, HI_U32 u32Align, VB_CAL_CONFIG_S *pstCalConfig)
{
    ...
    if (u32Align == 0) {
        ...
        u32Align = (HI_ALIGN_UP(u32Align, DEFAULT_ALIGN));
    }
    ...
    u32AlignHeight = HI_ALIGN_UP(u32Height, 2);

    if (enCmpMode == COMPRESS_MODE_NONE) {
        u32MainStride = HI_ALIGN_UP((u32Width * u32BitWidth + 7) >> 3, u32Align);
        ...
        if (u32BitWidth == 8) {
            u32MainStride = HI_ALIGN_UP(u32Width, u32Align);
        ...
        u32HeadSize = HI_ALIGN_UP(u32HeadSize, u32Align);
                                             ...
                                             COMPRESS_MODE_E enCmpMode, HI_U32 u32CmpRatio, HI_U32 u32Align)
{
    ...
    if (enCmpMode == COMPRESS_MODE_NONE) {
        u32Stride = HI_ALIGN_UP(HI_ALIGN_UP(u32Width * u32BitWidth, 8) / 8, u32Align);
        u32Size = u32Stride * u32Height;
    } else if (enCmpMode == COMPRESS_MODE_LINE) {
        HI_U32 u32Tmp;
        u32Tmp = HI_ALIGN_UP((16 + u32Width * u32BitWidth * 1000UL / u32RawCmpRatio + 8192 + 127) / 128, 2);
        u32Stride = HI_ALIGN_UP(u32Tmp * 16, u32Align);
        u32Size = u32Stride * u32Height;
    } else if (enCmpMode == COMPRESS_MODE_FRAME) {
        u32Size = HI_ALIGN_UP(u32Height * u32Width * u32BitWidth * 1000UL / (u32RawCmpRatio * 8), u32Align);
    }
...
__inline static HI_U32 VDEC_GetPicBufferSize(PAYLOAD_TYPE_E enType, HI_U32 u32Width,
    HI_U32 u32Height, __attribute__((unused))PIXEL_FORMAT_E enPixelFormat,
    __attribute__((unused))DATA_BITWIDTH_E enBitWidth,
    __attribute__((unused))HI_U32 u32Align)
{
    ...
    if (enType == PT_H264) {
        u32AlignWidth = HI_ALIGN_UP(u32Width, H264D_ALIGN_W);
        u32AlignHeight = HI_ALIGN_UP(u32Height, H264D_ALIGN_H);
        u32Size = ((u32AlignWidth * u32AlignHeight) * 3) >> 1;
    } else if (enType == PT_H265) {
        u32AlignWidth = HI_ALIGN_UP(u32Width, H265D_ALIGN_W);
        u32AlignHeight = HI_ALIGN_UP(u32Height, H265D_ALIGN_H);
        u32Size = ((u32AlignWidth * u32AlignHeight) * 3) >> 1;
    } else if ((enType == PT_JPEG) || (enType == PT_MJPEG)) {
        /* for PIXEL_FORMAT_YVU_SEMIPLANAR_420 */
        u32AlignWidth = HI_ALIGN_UP(u32Width, JPEGD_ALIGN_W);
        u32AlignHeight = HI_ALIGN_UP(u32Height, JPEGD_ALIGN_H);
        u32Size = (u32AlignWidth * u32AlignHeight * 3) >> 1;
    } else {
        u32Size = 0;
    }
...
__inline static HI_U32 VDEC_GetTmvBufferSize(PAYLOAD_TYPE_E enType, HI_U32 u32Width, HI_U32 u32Height)
{
    ...
    if (enType == PT_H264) {
        WidthInMb = HI_ALIGN_UP(u32Width, 16) >> 4;
        HeightInMb = HI_ALIGN_UP(u32Height, 16) >> 4;
        ColMbSize = 16 * 4;
        u32Size = HI_ALIGN_UP((ColMbSize * WidthInMb * HeightInMb), 128);
    } else if (enType == PT_H265) {
        WidthInMb = HI_ALIGN_UP(u32Width, 64) >> 4;
        HeightInMb = HI_ALIGN_UP(u32Height, 64) >> 4;
        ColMbSize = 4 * 4;
        u32Size = HI_ALIGN_UP((ColMbSize * WidthInMb * HeightInMb), 128);
    } else {
        u32Size = 0;
    ...
    __attribute__((unused))HI_U32 u32Align)
{
    ...
    if (enType == PT_H265) {
        u32AlignWidth = HI_ALIGN_UP(u32Width, 64) >> 6;
        u32AlignHeight = HI_ALIGN_UP(u32Height, 64) >> 6;

        u32TmvSize = u32AlignWidth * u32AlignHeight << 8;
        u32PmeSize = (u32AlignWidth << 4) * (u32AlignHeight << 4);

        u32AlignWidth = HI_ALIGN_UP(u32Width, 1024) >> 10;
        u32AlignHeight = HI_ALIGN_UP(u32Height, 64) >> 6;
        ...
        u32AlignWidth = HI_ALIGN_UP(u32Width, 16) >> 4;
        u32AlignHeight = HI_ALIGN_UP(u32Height, 16) >> 4;
        ...
        u32AlignWidth = HI_ALIGN_UP(u32Width, 4096) >> 12;
        u32AlignHeight = HI_ALIGN_UP(u32Height, 16) >> 4;
        u32PmeInfoSize = (u32AlignWidth * u32AlignHeight) << 5;
...
__inline static HI_U32 VENC_GetRefBufferSize(PAYLOAD_TYPE_E enType, HI_U32 u32Width, HI_U32 u32Height,
    DATA_BITWIDTH_E enBitWidth, __attribute__((unused))HI_U32 u32Align)
{
    HI_U32 u32Size = 0;
