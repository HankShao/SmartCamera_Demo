...
typedef ISP_SNS_MODE_E hi_isp_sns_mode;
...
typedef struct {
    ...
    hi_color_gamut color_gamut; /* RW; Range: [0, 3]; Color gamut type; COLOR_GAMUT_BT2020 only used for Hi3559AV100 */
...
typedef struct {
    hi_isp_people_roi  people_roi[PEOPLE_CLASS_MAX];
    hi_isp_tunnel_roi  tunnel_roi[TUNNEL_CLASS_MAX];
} hi_isp_smart_info;

typedef ISP_PEOPLE_TYPE_E hi_isp_people_type;
...
typedef struct {
    hi_u32  sns_vmax_addr[ISP_MAX_SNS_EXP_ADDR_NUM];  /* 3 addr high  mid  low */
...
typedef struct {
    ...
    hi_isp_mcf_data mcf_data; /* only for hi3516dv200/hi3516ev300 */
